...
@Component({
  template: `
    <table>
      <thead>
        <tr cplSort [(predicate)]="predicate" [(ascending)]="ascending" [callback]="transition.bind(this)">
          <th cplSortBy="name">ID<fa-icon [icon]="'sort'"></fa-icon></th>
        </tr>
      </thead>
    </table>
  `,
...
describe('Directive: SortByDirective', () => {
  ...
  it('should initialize predicate, order, icon when initial component predicate is _score', () => {
    ...
    // THEN
    expect(sortByDirective.cplSortBy).toEqual('name');
